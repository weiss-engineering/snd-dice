 ...
 * @dice
 * @cmd: command header
 * @operands: buffer containing command operands (will be appended to command
 * header)
 * @op_size: operands buffer size (bytes)
 * @response: buffer to copy response data to
 * @resp_size: size of response data to be copied
 * @resp_match_bytes: a bitmap specifying the response bytes used to detect the
 * correct response frame (apart from the command header which is already
 * checked (bits1-9)), this bitmap will be shifted accordingly (i.e. use
 * BIT(0...) to address the bytes within the response buffer).
 */
static int dice_avc_vendor_spec_cmd(struct snd_dice *dice,
				    ...
				    unsigned int resp_match_bytes)
{
	...
	buf[1] = (AVC_SU_TYPE_UNIT << 3) |
		 AVC_SU_ID_IGNORE; //(cmd->cmd.cmd.subunit_type<<3)
	//| cmd->cmd.cmd.subunit_id;
	...
	if (operands && op_size > 0) {
		memcpy(&buf[sizeof(struct avc_su_tc_vendor_cmd)], operands,
		       op_size);
	}

	err = fcp_avc_transaction(
		dice->unit, buf, tx_size, buf, rx_size,
		BIT(1) | BIT(2) | BIT(3) | BIT(4) | BIT(5) | BIT(6) | BIT(7) |
			BIT(8) | BIT(9) |
			(resp_match_bytes
			 << (sizeof(struct avc_su_tc_vendor_cmd) - 1)));
	if (err < 0) {
	...
	if (err < rx_size) {
		dev_err(&dice->unit->device,
			"short FCP response (%#x != %#lx)\n", err, rx_size);
		err = -EIO;
		goto error;
	}
	if (buf[0] != ((cmd->cmd.cmd.ctype == AVC_CTYPE_CONTROL) ?
			       AVC_RESPONSE_ACCEPTED :
			       AVC_RESPONSE_STABLE)) {
		dev_err(&dice->unit->device,
			"vendor read command failed (%#x)\n", buf[0]);
		err = -EIO;
	}
...
static int weiss_dice_write_param(struct snd_dice *dice,
				  struct weiss_cmd_param_op *param)
{
	...
	err = dice_avc_vendor_spec_cmd(
		dice, &cmd, param, sizeof(struct weiss_cmd_param_op), NULL, 0,
...
static int weiss_dice_read_param(struct snd_dice *dice,
				 struct weiss_cmd_param_op *param)
{
	int err;
	u8 i;
	...
	err = dice_avc_vendor_spec_cmd(
		dice, &cmd, param, sizeof(struct weiss_cmd_param_op), param,
		sizeof(struct weiss_cmd_param_op),
		BIT(0) | BIT(1) | BIT(2) | BIT(3)); /*param ID: first 4 bytes*/
	if (err < 0) {
...
static int weiss_dice_dev_const(struct snd_dice *dice,
				struct weiss_cmd_dev_const *dev_const)
{
	...
	err = dice_avc_vendor_spec_cmd(dice, &cmd, dev_const,
				       sizeof(struct weiss_cmd_dev_const),
				       dev_const,
				       sizeof(struct weiss_cmd_dev_const), 0);
	if (err < 0) {
		dev_err(&dice->unit->device,
			"AVC dev_const read failed (%i).\n", err);
		return err;
	}
	for (i = 0; i < sizeof(struct weiss_cmd_dev_const) / 4; ++i) {
		be32_to_cpus(&((u32 *)dev_const)[i]);
	}
	_dev_info(&dice->unit->device,
		  "Weiss device constitution: params:%#x,attrs:%#x\n",
		  dev_const->num_params, dev_const->num_attrs);
	return err;
}
static int weiss_dice_param_info(struct snd_dice *dice,
				 struct weiss_cmd_param_info *param_info)
{
	...
	err = dice_avc_vendor_spec_cmd(
		dice, &cmd, param_info, sizeof(struct weiss_cmd_param_info),
		param_info, sizeof(struct weiss_cmd_param_info),
		BIT(0) | BIT(1) | BIT(2) | BIT(3)); /*param ID: first 4 bytes*/
	if (err < 0) {
		dev_err(&dice->unit->device,
			"AVC param_info read failed (%i).\n", err);
		return err;
	}
	for (i = 0; i < sizeof(struct weiss_cmd_param_info) / 4; ++i) {
		be32_to_cpus(&((u32 *)param_info)[i]);
	}
	//	_dev_info(&dice->unit->device, "read param info
	//(ID:%#x,name:'%s',iface:%#x,type:%#x,\n", 			param_info->param_id,
	//param_info->name, param_info->iface, param_info->type);
	...
	//				param_info->integer.min,
	//param_info->integer.max); 		break; 	case SNDRV_CTL_ELEM_TYPE_BOOLEAN: 		break;
	//	default:
	//		dev_err(&dice->unit->device, "unsupported parameter type (%#x)\n",
	//param_info->type); 		return -EINVAL;
	//	}
	return err;
}
static int weiss_dice_enum_item_info(struct snd_dice *dice,
				     struct weiss_cmd_enum_item_info *item_info)
{
	...
	err = dice_avc_vendor_spec_cmd(
		dice, &cmd, item_info, sizeof(struct weiss_cmd_enum_item_info),
		item_info, sizeof(struct weiss_cmd_enum_item_info),
		BIT(0) | BIT(1) | BIT(2) | BIT(3) | BIT(4) | BIT(5) | BIT(6) |
			BIT(7)); /*param&item ID: first 8 bytes*/
	if (err < 0) {
		dev_err(&dice->unit->device,
			"AVC item_info read failed (%i).\n", err);
		return err;
	}
	for (i = 0; i < sizeof(struct weiss_cmd_enum_item_info) / 4; ++i) {
		be32_to_cpus(&((u32 *)item_info)[i]);
	}
	//	_dev_info(&dice->unit->device, "read item info
	//(paramID:%#x,itemID:%#x,name:'%s')\n", 			item_info->param_id,
	//item_info->item_id, item_info->name);
	return err;
}
...
{
	static char *texts[13] = { "AES1", "AES2", "AES3",	"AES4", "AES",
				   "ADAT", "TDIF", "Wordclock", "ARX1", "ARX2",
				   "ARX3", "ARX4", "Internal" };
			     ...
			     struct snd_ctl_elem_value *ucontrol)
{
	...
	err = snd_dice_transaction_read_global(dice, GLOBAL_CLOCK_SELECT, &reg,
					       sizeof(reg));
	if (err < 0) {
		return err;
	}
			     ...
			     struct snd_ctl_elem_value *ucontrol)
{
	...
	err = snd_dice_transaction_write_global(dice, GLOBAL_CLOCK_SELECT, &new,
						sizeof(new));
	if (err < 0)
		return err;
...
static int dice_sync_rate_info(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_info *uinfo)
{
	static char *texts[11] = { "32000",   "44100",	  "48000",  "88200",
				   "96000",   "176400",	  "192000", "ANY_LOW",
				   "ANY_MID", "ANY_HIGH", "NONE" };
	uinfo->type = SNDRV_CTL_ELEM_TYPE_ENUMERATED;
	uinfo->count = 1;
			      ...
			      struct snd_ctl_elem_value *ucontrol)
{
	...
	int err = 0;
	__be32 reg, new;
	u32 value = (ucontrol->value.enumerated.item[0] << CLOCK_RATE_SHIFT) &
		    CLOCK_RATE_MASK;
	u32 data;
				     ...
				     struct snd_ctl_elem_info *uinfo)
{
	...
	err = weiss_dice_param_info(dice, &param_info);
	if (err < 0) {
		dev_err(&dice->unit->device, "fail to get param info (%i)",
			err);
		return err;
	}
...
static int dice_weiss_snd_ctl_construct(struct snd_dice *dice)
{
	...
	for (i = 0; i < dev_const.num_params; ++i) {
		...
		case SNDRV_CTL_ELEM_TYPE_ENUMERATED: {
			...
			err = snd_ctl_add(dice->card,
					  snd_ctl_new1(&control, dice));
			if (err < 0) {
				return err;
			}
		} break;
